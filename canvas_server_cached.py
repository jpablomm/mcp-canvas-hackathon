...
# Constants
API_BASE_URL = os.environ.get("CANVAS_API_URL", "https://canvas.berkeley.edu/api/v1")
...
async def assign_peer_review(
    ...
    if not reviewee_submission:
        ...
        if "error" in reviewee_submission:
            return (
                f"Error creating placeholder submission: {reviewee_submission['error']}"
            )
    ...
    data = {"user_id": reviewer_id}  # The user who will do the review
...
async def list_peer_reviews(course_identifier: str, assignment_id: str) -> str:
    ...
    users = await fetch_all_paginated_results(
        f"/courses/{course_id}/users", {"per_page": 100}
    )
    ...
    course_display = await get_course_code(course_id) or course_identifier
    output = (
        f"Peer Reviews for Assignment {assignment_id} in course {course_display}:\n\n"
    )
...
async def get_student_analytics(
    ...
    students = await fetch_all_paginated_results(f"/courses/{course_id}/users", params)
    ...
    for student in students:
        ...
        if include_participation or include_access_stats:
            ...
            if (
                not isinstance(participation_data, dict)
                or "error" not in participation_data
            ):
                if include_participation:
                    analytics["participation"] = {
                        ...
                        "total_activity_time": participation_data.get(
                ...
                if include_access_stats:
                    ...
                    analytics["access"] = {
                        ...
                        "access_pattern": (
                            "Regular" if len(view_data) > 5 else "Irregular"
                        ),
                    }
            ...
            if not isinstance(assignment_data, dict) or "error" not in assignment_data:
                ...
                if current_only:
                    ...
                    for assign in assignment_data:
                        ...
                        if any(
                            a.get("id") == assignment_id for a in current_assignments
                        ):
                ...
                total_assignments = (
                    len(filtered_assignment_data) if filtered_assignment_data else 0
                )
                ...
                completion_rate = (
                    (effective_submitted / total_assignments * 100)
        ...
        if include_assignment_stats and student.get("assignments"):
            ...
            if current_only:
                ...
                if a.get("missing", 0) > 0 and a.get("assignment_status"):
                    missing_assignments = [
                        ass
                        for ass in a.get("assignment_status", [])
                        if ass.get("missing")
                    ]
                    if missing_assignments:
                        ...
                        for ma in missing_assignments[
                            :3
                        ]:  # Limit to first 3 to keep output reasonable
                            due_date = ma.get("due_date", "Unknown")
                            if due_date and due_date != "Unknown":
                                due_date = due_date.split("T")[
                                    0
    ...
    if include_assignment_stats and student_analytics:
        avg_completion = (
            sum(s["assignments"].get("completion_rate", 0) for s in student_analytics)
            / len(student_analytics)
            if student_analytics
            else 0
        )
        avg_graded = (
            sum(s["assignments"].get("graded", 0) for s in student_analytics)
            / len(student_analytics)
            if student_analytics
            else 0
        )
        avg_score = (
            sum(s["assignments"].get("average_score", 0) for s in student_analytics)
            / len(student_analytics)
            if student_analytics
            else 0
        )
        ...
        if current_only:
            ...
            total_current_assignments = (
                len(filtered_assignments) if filtered_assignments else 0
            )

            # Calculate global stats
            graded_count = sum(
                s["assignments"].get("graded", 0) for s in student_analytics
            )
            submitted_count = sum(
                s["assignments"].get("submitted", 0) for s in student_analytics
            )
            effective_count = sum(
                s["assignments"].get("effective_submitted", 0)
                for s in student_analytics
            )
            missing_count = sum(
                s["assignments"].get("missing", 0) for s in student_analytics
            )

            # Calculate maximum possible number of all student/assignment combinations
            total_possible = (
                total_students * total_current_assignments
                if total_current_assignments > 0
                else 0
            )

            output += f"Average Current Assignment Completion Rate: {round(avg_completion, 1)}%\n"
            output += (
                f"Average Graded Assignments Per Student: {round(avg_graded, 1)}\n"
            )
            ...
            output += (
                f"Average Assignment Completion Rate: {round(avg_completion, 1)}%\n"
            )
            output += (
                f"Average Graded Assignments Per Student: {round(avg_graded, 1)}\n"
            )
        ...
        if filtered_assignments:
            ...
            for assignment in filtered_assignments:
                ...
                if assignment_due_date and assignment_due_date != "Unknown":
                    assignment_due_date = assignment_due_date.split("T")[
                        0
                    ]  # Just show the date part
                ...
                for student in student_analytics:
                    ...
                    if (
                        "assignments" in student
                        and "assignment_status" in student["assignments"]
                    ):
                        for status in student["assignments"]["assignment_status"]:
                            if status.get("id") == assignment_id and status.get(
                                "submitted"
                            ):
                                submission_count += 1
                                break
                ...
                completion_rate = (
                    (submission_count / total_students * 100)
                    if total_students > 0
                    else 0
                )
            ...
            lowest_completion = sorted(
                assignment_completion.items(), key=lambda x: x[1]
            )[:3]
            if lowest_completion:
                ...
                for name, rate in lowest_completion:
                    ...
                    output += (
                        f"  {name} (Due: {due_date}): {round(rate, 1)}% completion\n"
                    )
...
async def get_assignment_analytics(course_identifier: str, assignment_id: str) -> str:
    ...
    course_display = await get_course_code(course_id) or course_identifier
    output = (
        f"Assignment Analytics for '{assignment_name}' in Course {course_display}\n\n"
    )
